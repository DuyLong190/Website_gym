...
<body>
    <div class="main-content">
        <div class="page-header">
            <h1>
                ...
                <div class="title-text">
                    <span class="title-main">Quản lý gói tập</span>
                </div>
            </h1>
        </div>
        <div class="card admin-card">
            <div class="card-header">
                ...
                <button class="btn btn-add-user" data-bs-toggle="modal" data-bs-target="#addGoiTapModal" title="Thêm gói tập">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    ?>
                    <table id="goiTapTable" class="table table-bordered table-striped align-middle">
                        <thead>
                            <tr>
                                <th style="width: 12%;" class="text-center">Tên gói tập</th>
                                <th style="width: 11%;" class="text-center">Giá</th>
                                <th style="width: 12%;" class="text-center">Thời hạn</th>
                        ...
                        <tbody>
                            <?php if (!empty($goiTaps)): ?>
                                <?php foreach ($goiTaps as $goiTap): ?>
                                    <tr>
                                        <td class="profile-value"><?php echo htmlspecialchars($goiTap['TenGoiTap'] ?? 'Chưa có gói tập'); ?></td>
                                        <td class="profile-value"><?php echo isset($goiTap['GiaTien']) ? number_format($goiTap['GiaTien'], 0, ',', '.') . ' VNĐ' : 'N/A'; ?></td>
                                        <td class="profile-value"><?php echo isset($goiTap['ThoiHan']) ? $goiTap['ThoiHan'] . ' tháng' : 'N/A'; ?></td>
                                        <td class="text-center">
                                            ...
                                            <button onclick="confirmDeleteGoiTap(<?php echo $goiTap['MaGoiTap'] ?? 0; ?>, '<?php echo htmlspecialchars($goiTap['TenGoiTap'] ?? 'Chưa có gói tập', ENT_QUOTES); ?>')" 
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center text-muted">Không có gói tập nào.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addGoiTapModalLabel">
                        <i class="fas fa-ticket-alt"></i>
                        Thêm Gói tập mới
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="/gym/admin/goitap/saveGoiTap" method="POST" id="addGoiTapForm">
                    <div class="modal-body">
                        <div class="form-group-enhanced">
                            ...
                            <input type="text" class="form-control" name="TenGoiTap" required placeholder="Nhập tên gói tập">
                        </div>
                        <div class="form-group-enhanced">
                            <label class="form-label">
                            ...
                            <input type="number" class="form-control" name="GiaTien" required min="0" placeholder="Nhập giá tiền">
                        </div>
                        <div class="form-group-enhanced">
                            <label class="form-label">
                            ...
                            <input type="number" class="form-control" name="ThoiHan" required min="1" placeholder="Nhập thời hạn">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">
                            ...
                            <textarea class="form-control" name="MoTa" rows="4" placeholder="Nhập mô tả"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="me-auto btn btn-primary" id="submit" name="submit">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteConfirmModalLabel">
                        ...
                        <span>Xác nhận xóa gói tập</span>
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="warning-text">
                        Bạn có chắc chắn muốn xóa gói tập này?<br>
                        <strong class="text-danger">Thao tác này không thể hoàn tác!</strong>
                    </div>
                    <div class="account-info">
                        <div class="account-info-label">Tên gói tập</div>
                        <div class="account-info-value" id="deleteGoiTapName"></div>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        let goiTapIdToDelete = null;

        function confirmDeleteGoiTap(id, name) {
            goiTapIdToDelete = id;
            
            // Cập nhật thông tin gói tập trong modal
            document.getElementById('deleteGoiTapName').textContent = name;
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            if (goiTapIdToDelete) {
                window.location.href = '/gym/admin/goitap/deleteGoiTap/' + goiTapIdToDelete;
            }
        });

        // Reset khi modal đóng
        document.getElementById('deleteConfirmModal').addEventListener('hidden.bs.modal', function() {
            goiTapIdToDelete = null;
        });

        // Modal enhancement
        $(document).ready(function() {
            // Reset form when modal is closed
            $('#addGoiTapModal').on('hidden.bs.modal', function() {
                $(this).find('form')[0].reset();
                $(this).find('.form-control').removeClass('is-invalid is-valid');
            });

            // Form validation with visual feedback
            $('#addGoiTapForm').on('submit', function(e) {
                let isValid = true;
            ...
            $('#addGoiTapForm input[required], #addGoiTapForm textarea[required]').on('blur', function() {
                if ($(this).val().trim()) {
                    $(this).addClass('is-valid').removeClass('is-invalid');
                } else {
                    $(this).addClass('is-invalid').removeClass('is-valid');
